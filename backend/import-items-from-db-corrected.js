...
// Helper function to clean description (remove Grade)
function cleanDescription(description) {
  if (!description) return null;
  // Remove (Grade: X) pattern from description
  return description.replace(/\s*\(Grade:\s*[A-Z0-9]+\)\s*/gi, '').trim() || null;
}
...
(async () => {
  try {
    console.log('==========================================');
    console.log('  IMPORT DATA FROM items-only.db');
    console.log('  Corrected Import - Removing Grade, Fixing Costs/Prices');
    console.log('==========================================\n');

    const sourceDbPath = '/var/www/Dev-Koncepts/items-only.db';
    ...
    console.log('Source database counts:');
    ...
    // ============================================
    // Step 6: Import Parts (CORRECTED - Remove Grade, Fix Costs/Prices)
    // ============================================
    console.log('Step 6: Importing Parts (Corrected - Removing Grade, Fixing Costs/Prices)...');
    ...
    for (const part of parts) {
      try {
        ...
        // Clean description - remove Grade
        const originalDescription = part.description || null;
        const cleanedDescription = cleanDescription(originalDescription);
        if (originalDescription && originalDescription !== cleanedDescription) {
          cleanedDescriptions++;
        }

        // Safely convert numeric fields
        const cost = safeNumber(part.cost);
        const priceA = safeNumber(part.priceA);
        const priceB = safeNumber(part.priceB);
        const priceM = safeNumber(part.priceM);
        const weight = safeNumber(part.weight);
        const reorderLevel = part.reorderLevel ? parseInt(part.reorderLevel) : 0;
        ...
        await prisma.part.upsert({
          ...
          update: {
            ...
            partNo: part.partNo,
            brandId: brandId,
            description: cleanedDescription,  // Cleaned description without Grade
            categoryId: categoryId,
            subcategoryId: subcategoryId,
            ...
            uom: part.uom || 'pcs',
            cost: cost,  // Correctly imported cost
            priceA: priceA,  // Correctly imported priceA
            priceB: priceB,  // Correctly imported priceB
            priceM: priceM,  // Correctly imported priceM
            smc: part.smc || null,
            size: part.size || null,
            origin: part.origin || null,
            imageP1: part.imageP1 != null ? part.imageP1 : null,  // Preserving images accurately
            imageP2: part.imageP2 != null ? part.imageP2 : null,  // Preserving images accurately
    ...
    console.log(`  ✓ Cleaned ${cleanedDescriptions} descriptions (removed Grade)\n`);
    ...
    // Check for cleaned descriptions (SQLite doesn't support case-insensitive, but we'll check anyway)
    const partsWithGrade = await prisma.part.count({
      where: {
        description: {
          contains: 'Grade'
        }
    ...
    // Check for cost and price data
    const partsWithCost = await prisma.part.count({
      where: {
        cost: { not: null }
      }
    });

    const partsWithPriceA = await prisma.part.count({
      where: {
        priceA: { not: null }
      }
    });
    ...
    console.log(`  Parts with Grade in description: ${partsWithGrade} (should be 0)`);
    console.log(`  Parts with Cost: ${partsWithCost}`);
    console.log(`  Parts with PriceA: ${partsWithPriceA}`);
    console.log('');
    ...
    if (sampleParts.length > 0) {
      console.log('Sample imported parts (verification):');
      sampleParts.forEach(part => {
        console.log(`  ${part.partNo}: ${part.description || '(no description)'}`);
        console.log(`    Cost: ${part.cost}, PriceA: ${part.priceA}, PriceB: ${part.priceB}`);
      });
      console.log('');
    }

    if (partsWithGrade === 0) {
      console.log('✅ SUCCESS! All data imported accurately:');
      console.log('   ✓ Grade removed from descriptions');
      console.log('   ✓ Costs and Prices imported correctly');
      console.log('   ✓ Images preserved');
      console.log('   ✓ All other details correct');
    } else {
      console.log(`⚠️  Warning: ${partsWithGrade} parts still have Grade in description`);
    }
